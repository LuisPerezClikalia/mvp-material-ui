...
function muiError({ references, babel, config, source }) {
  ...
  function evaluateMessage(node) {
    if (babel.types.isBinaryExpression(node)) {
      if (node.operator !== "+") {
        throw new Error(
          `Unsupported binary operator '${node.operator}'. Can only evaluate '+'.`
        );
      }
      return `${evaluateMessage(node.left, babel)}${evaluateMessage(
        node.right,
        babel
      )}`;
    }
  ...
  references.default.forEach((babelPath) => {
    ...
    const errorMessageLiteral = evaluateMessage(
      newExpressionPath.node.arguments[0]
    );
    ...
    const devMessage = babel.types.templateLiteral(
      errorMessageQuasis,
    ...
    if (formatMuiErrorMessageIdentifier === null) {
      const isBareImportSourceIdentifier = source.startsWith(
      ...
      if (isBareImportSourceIdentifier) {
        ...
        throw new Error(
          "Only package imports from @mui/internal-babel-macros are supported"
        );
      }
    }
  ...
  if (missingError === "write" && updatedErrorCodes) {
    fs.writeFileSync(
      errorCodesPath,
      JSON.stringify(invertObject(errorCodesLookup), null, 2)
    );
  }
